...
interface SettingsPanelProps {
    ...
    setParticleOptions: React.Dispatch<React.SetStateAction<Record<string, string | number> | null>>;
  }
  
  const SettingsPanel: React.FC<SettingsPanelProps> = ({ setSelectedEffect, setParticleOptions }) => {
    const effects = (effectsData as EffectsData).effects;
    const firstEffect = effects[0];
    const [applyWithoutOptions, setApplyWithoutOptions] = useState(true);
    const [localSelectedEffect, setLocalSelectedEffect] = useState<Effect | null>(firstEffect);
    const [localSettings, setLocalSettings] = useState<Record<string, string | number>>(
      Object.fromEntries(firstEffect.properties.map(prop => [prop.name, prop.default]))
    );
    ...
    useEffect(() => {
      if (localSelectedEffect) {
        const initialSettings = Object.fromEntries(localSelectedEffect.properties.map(prop => [prop.name, prop.default]));
  ...
  useEffect(() => {
    ...
    return (
        <div className="border-2 border-gray-300 rounded-lg h-[90vh] w-[25vw] p-6">
          <DropdownMenu>
            <DropdownMenuTrigger className="outline outline-1  text-white py-2 px-2 rounded">Choose Effect</DropdownMenuTrigger>
            <DropdownMenuContent className="rounded-md shadow-lg">
              {effects.map((effect, index) => (
                <DropdownMenuItem key={index} onSelect={() => handleEffectChange(effect.name)} className="px-4 py-2 hover:bg-blue-200">
                  {effect.name}
                </DropdownMenuItem>
              ))}
            </DropdownMenuContent>
...
/>
    <span className="ml-2">Apply particle options</span>
      ...
      {localSelectedEffect && (
        <div className="mt-8">
          <h2 className="text-2xl font-semibold mb-5">{localSelectedEffect.name} Settings</h2>
          {localSelectedEffect.properties.map((prop, index) => {
            switch (prop.type) {
              case 'number':
                return (
                  <div key={index} className="mb-4">
                    <Label htmlFor={`${prop.name}-input`} className="block text-sm font-medium text-gray-700">{prop.name}</Label>
                    <Input
                      type="number"
                      id={`${prop.name}-input`}
                      value={localSettings[prop.name]}
                      onChange={(e) => setLocalSettings({ ...localSettings, [prop.name]: Number(e.target.value) })}
                ...
                return (
                  <div key={index} className="mb-4">
                    ...
                    <Input
                      id={`${prop.name}-input`}
                      value={localSettings[prop.name]}
                      onChange={(e) => setLocalSettings({ ...localSettings, [prop.name]: e.target.value })}
                      className="mt-1 p-2 border rounded"
                    />
                  </div>
                );
        ...
        <div className="italic text-sm text-gray-500">
          {localSelectedEffect.properties.map((prop) => prop.note && <div key={prop.name}>{prop.note}</div>)}
        </div>
        </div>
      )}
    </div>
